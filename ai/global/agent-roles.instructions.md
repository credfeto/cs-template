...
3. On failure (any language's branch coverage below its baseline): the procedure posts a `<lang> X% < main Y% - returning to Development` status comment and moves the board back to **Development**; **STOP** here, the next cycle picks the resulting Development work back up.
