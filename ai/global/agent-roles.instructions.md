...
- **P5.** **Live-chat approval ends the wait immediately.** If the human's chat message opens with the literal word `approved` or `lgtm` (case-insensitive) and is otherwise an unconditional approval, do not wait for the next tick. This is the one place the agent acts on a chat message alone. A question ("is this approved yet?"), a negation ("not approved"), a qualified approval or a passing mention does not count; if in doubt, ask:
  ...
  4. If board data is present, set `Workflow Status` to **Approved** with `cfwf workflow-status --set ... --status Approved` (see [Workflow Board](#workflow-board)).
...
**`cfwf` does not verify a write, deliberately.** The GitHub API can lag behind a write by seconds, so a read straight afterwards cannot tell a lost write from lag. An earlier read-back inside `--set` produced false "did not persist" failures for writes that had persisted, and caught no lost writes. Anyone who wants to confirm a status uses `cfwf workflow-status --check`, after allowing for the lag (see [GitHub State Lags Behind Writes](github-cli.instructions.md#github-state-lags-behind-writes-mandatory)): do not re-check straight away or repeat the write. The read-back rule this replaces was drawn from `funfair-tech/funfair-server-template` issue #918, fixed in PR #920, where a write that looked successful was reported as done and went unnoticed because nothing threw; that incident is history, not a requirement to verify. `--check` reads the board with a read-only GraphQL query rather than `gh project item-list`, because `gh project item-list` lags behind writes and is capped. That query is a documented, narrow exception to [the preference for native `gh` subcommands](github-cli.instructions.md#rest-and-graphql-api-gh-api).
