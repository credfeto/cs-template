...
- **Skip case**: if the work item qualifies for a skip under [changelog.instructions.md](changelog.instructions.md#when-to-skip) (template repo, docs-only, AI-instruction-only change), skip Placeholder entirely — there is nothing to commit yet, so Committer/PR Submitter cannot open a PR from an empty branch. Code Writer runs first as before, and PR Submitter opens the PR from that first real commit. Correction is likewise a no-op for these items.
