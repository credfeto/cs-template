...
- Before fixing a round's findings, group them by construct; each group gets one fix commit and one sweep. One sweep, and one sweep commit, per construct, not per finding or per occurrence: when several findings report the same construct at different sites, one sweep covers them all. Skip the sweep if this PR already carries a sweep commit for the same construct (its body names the construct) and no later commit reintroduced it; a site a later review round deliberately reverted is not reintroduced.
- Sweep in the working tree before handing off for build/test verification, so one run covers both the fix and the sweep; then commit fix first, sweep second, in the same PR. The fix commit must build and pass tests on its own: anything a fix-touched file depends on is part of the fix, not the sweep. Exception: Phase A of the [PR review loop](agent-roles.instructions.md#pr-workflow-ai-review-loop) sweeps once after `/simplify` converges rather than after each round.
- A sweep change set handed to a committing role is the list of files touched only by the sweep plus the per-file rationale for its commit body; the producing role records it in its hand-off report and every intermediate role passes it on unchanged. A sweep hunk in a file the fix also touches goes into the fix commit and is listed in that commit's body with the same rationale. When the origin commits are already pushed (a Phase A sweep), every hunk goes into the sweep commit.
...
- A sweep that would touch more than 25 files is not applied silently: post the site list on the PR, add `Blocked`, and wait for a human decision on whether it belongs in this PR or a follow-up issue.
...
- If the sweep finds nothing, record `Swept: no other occurrences of <construct>` in the fix commit body; no sweep commit or extra comment is needed.
