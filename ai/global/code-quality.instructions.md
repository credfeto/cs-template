...
- Sweep in the working tree before handing off for build/test verification, so one run covers both the fix and the sweep; then commit fix first, sweep second, in the same PR. Exception: Phase A of the [PR review loop](agent-roles.instructions.md#pr-workflow-ai-review-loop) sweeps once after `/simplify` converges rather than after each round.
