...
- The sweep record handed to a committing role is: a `Construct:` line naming the construct searched for, the finding or comment reference, and one line per file touched only by the sweep with why that site matches (or `Swept: none` when nothing was found). The producing role appends it to its hand-off report, and every intermediate role (Code Tester, Code Reviewer, Changelog) carries it in its own report unchanged. A sweep hunk in a file the fix also touches goes into the fix commit and is listed in that commit's body with the same rationale. When the origin commits are already pushed (a Phase A sweep), every hunk goes into the sweep commit.
...
- If the sweep finds nothing, the fix commit body carries the `Construct:` line and `Swept: none`; no sweep commit or extra comment is needed.
