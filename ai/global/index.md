...
| [github-cli.instructions.md](github-cli.instructions.md) | Running any `gh` command | Full `gh` command reference (issues, PRs, labels, search, api/graphql, proxy behaviour, HEREDOC bodies), the rule to raise a `credfeto-orchestrator` issue for any `--json`/`gh api graphql` use `cfwf` does not cover, GitHub state lagging behind writes (do not spam while waiting), common mistakes learned from real failures |
