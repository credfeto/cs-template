...
| [shell.instructions.md](shell.instructions.md) | Shell scripting conventions: `firewall-cmd` helper functions (`allow_ipv4`, `allow_ipv6`, `open_port_for_private_networks`), private network constants, port-opening rules |
| [shell-scripts.instructions.md](shell-scripts.instructions.md) | Prefer `#!/bin/sh` over `#!/bin/bash`, shellcheck/checkbashisms compliance, coloured ✓/✗ visual indicators using `printf` |
