...
6. **On failure**, check the round cap first, then judge the round-over-round trend:
   - **Round cap**: if `MAX_COVERAGE_ITERATIONS` rounds have already run (counted from prior `... - returning to Development` coverage comments) without the branch catching up, whether or not this round's trend is itself closing: post a PR comment listing the still-failing languages and their gap, add `Blocked` label, and stop. Do not write `COVERAGE.md` in this case.
