...
**Bootstrap**: the [Pre-Work Baseline Check](git.instructions.md#pre-work-baseline-check-mandatory-before-starting-any-work) is expected to establish `COVERAGE.md` on `main` before any work branch reaches this phase, via its own dedicated branch — the same pattern as a pre-commit auto-fix commit — so a repo adopting this file for the first time gets it committed on its own, ahead of any feature/fix PR's diff. If the AI Coverage phase is nonetheless reached with `origin/main` still missing `COVERAGE.md` (that step was skipped, or this branch predates it), fall back to treating it the same way: there is nothing to compare against, this is not a regression, so it must not block. Generate it fresh from the branch's current measurements, treat the gate as passed, and proceed as in [step 5](#ai-coverage-phase-decision-procedure-mandatory) of the decision procedure below.
