...
`git commit`/`pre-commit`/`pre-commit-check`, `dotnet build`, `dotnet test`, `npm test`, and `bun test` have no bounded, predictable duration: `pre-commit` (and `pre-commit-check`, this template's wrapper that runs it against the existing checked-out repo) can run a heavy hook chain (`dotnet buildcheck` across every project, `trivy`, `hadolint`), `dotnet build` runs through a large analyzer stack (Roslynator, SonarAnalyzer, Meziantou, Threading, Security Code Scan, and more) plus NuGet restore, and test runs scale with what changed. A commit has already been killed mid-run on a foreground timeout in a live session. There is no timeout value that is both practical and safe to pick for any of these six commands, so do not try to pick one.

- **Always run these six commands via `run_in_background`; never in the foreground, regardless of how fast the specific run is expected to be.** This is unconditional, not a per-invocation judgement call.
- **Never wrap any of these six in a shell `timeout` command as a substitute or a belt-and-braces addition** (e.g. `timeout 590 dotnet test ...`), whether or not `run_in_background: true` is also set. `timeout` is on `reject-obfuscated-commands`' categorical blocklist (see [claude-hooks.instructions.md](claude-hooks.instructions.md#reference-installed-hook-set)) and is rejected outright, independently of the backgrounding rule above — plain `run_in_background: true` on the unwrapped command is already unbounded and needs no additional wrapper. Confirmed in practice (`credfeto/credfeto-notification-bot#280`): an agent added a `timeout N` wrapper around a backgrounded `dotnet test` call, got denied by the blocklist hook, and misdiagnosed it as "the permission system rejects `run_in_background`" — the two denials (missing `run_in_background`, and a banned `timeout` wrapper) come from different hooks and are unrelated; read which one actually fired rather than assuming a single general conflict.
