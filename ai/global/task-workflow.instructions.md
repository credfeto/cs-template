...
- **Always set an explicit maximum timeout on the tool call itself; never omit it and rely on the tool's built-in default.** Confirmed in practice: Claude Code's Bash tool defaults to 2 minutes when no `timeout` is given, which is well under typical pre-commit run times and will truncate the hook mid-run. Pass the maximum available timeout rather than a short one (e.g. Claude Code's Bash tool supports up to 600000ms/10 minutes; use it explicitly on every such call).
...
- **A long stretch with no new output is normal and is not a hang.** `pre-commit`, `dotnet buildcheck`, and similar hooks can run silently for minutes at a time. Do not interpret silence as a failure and manually cancel or kill the command on that basis. The only valid reasons to stop waiting are: the tool itself reports its timeout was hit, or a poll-loop deadline (see [Background Tasks and Monitor Tool](#background-tasks-and-monitor-tool-mandatory) above) actually fires.
- A killed run does not just fail; it skips the target process's own cleanup (a bash `EXIT` trap, .NET's `IDisposable` teardown, etc.), leaving orphaned temp directories, lock files, or half-applied state behind. Confirmed in practice: a killed `bats` run left thousands of orphaned fixture directories under a shared runtime directory, which went on to break an unrelated tool (`firejail`) that walked the same path; separately, a commit/pre-commit run has been killed on timeout in a live session despite this rule already existing, which is why the two bullets above were added.
